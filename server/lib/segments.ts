 ...
 *
 * Pure TypeScript on raw RGB; sharp is only used for decoding and drawing.
 */
import sharp from "sharp";
import type { NormPoint, SceneAnalysis, StoneSurface } from "../../shared/scene";
...
function toLab(rgb: Uint8Array | Buffer, n: number): Float32Array {
 ...
 */
export function segmentRgb(rgb: Uint8Array | Buffer, width: number, height: number, k: number, minArea: number): Segmentation {
 ...
 */
export function surfaceFromRegions(surface: StoneSurface, regions: number[], seg: Segmentation): StoneSurface | null {
  const { width: w, height: h } = seg;
  const set = consistentRegions([...new Set(regions.filter((r) => r >= 1 && r <= seg.count))], seg);
  ...
  const opened = morph(morph(mask, w, h, 2, "min"), w, h, 2, "max");
  let openedArea = 0;
